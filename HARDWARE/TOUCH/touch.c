...
{	
	...
	}else
	{
		...
		RCC->AHB1ENR|=1<<8;     //使能PORTI时钟 
 		GPIO_Set(GPIOG,PIN3,GPIO_MODE_IN,0,0,GPIO_PUPD_PU);	//PG3 设置为上拉输入
 		GPIO_Set(GPIOH,PIN7,GPIO_MODE_IN,0,0,GPIO_PUPD_PU);	//PH7 设置为上拉输入		
		GPIO_Set(GPIOI,PIN3|PIN8,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);	//PI3/PI8设置为推挽输出
